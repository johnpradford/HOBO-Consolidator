...
The launcher prefers `.venv\Scripts\python.exe` in this repo; if not found, it falls back to `py -3`. It injects `src` into `PYTHONPATH`, shows guidance when launched without inputs, and pauses on errors so messages stay visible.
